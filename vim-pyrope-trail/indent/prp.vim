...
" Set the function to do the work.
 setlocal indentexpr=GetPyropeIndent()
"
 let b:undo_indent = "set cin< cino< indentkeys< indentexpr<"
"
" " Only define the function once.
 if exists("*GetPyropeIndent")
 	finish
 endif
...
function! GetPyropeIndent()
	" Pyrope can use C indenting and then
	" correct the comment/precompiler duality.
	let theIndent = cindent(v:lnum)
	let prevLine = SkipPyropeComments(v:lnum-1)
	if (prevLine > 0) 
	...
	if getline(prevLine) =~ '(.*)$'
		...
		if getline(v:lnum) =~ '^[^{]*}'
	               " remove one indentation level, closing a block 
	        	let theIndent -= &sw
		endif
	endif	
  ...
  call Log("before comment - Indent is " . theIndent)
	...
	if getline(v:lnum) =~ '^\s*#'
        	...
        	if getline(prevLine) =~ '{$'							
                      "indent once more, because we're in a beginning of a
		      "block  
		      let theIndent += &sw
                elseif getline(prevLine) =~ '([^)]*$'
	               " indent twice more, because we're in a beginning of a parameters
                 	"" block which gets indented twice by cindent,
